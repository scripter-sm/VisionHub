...
for _, path in next, library.folders do
    ...
    local fonts = {}; do
        ...
        pcall(function()
            Medium = Register_Font("Vision_Medium", 200, "Normal", {
                Id = "Vision_Medium.ttf",
                Font = game:HttpGet("https://github.com/i77lhm/storage/raw/refs/heads/main/fonts/Inter_28pt-Medium.ttf"),
            })
            SemiBold = Register_Font("Vision_SemiBold", 200, "Normal", {
                Id = "Vision_SemiBold.ttf",
                Font = game:HttpGet("https://github.com/i77lhm/storage/raw/refs/heads/main/fonts/Inter_28pt-SemiBold.ttf"),
            })
        end)

        local function FontFace(asset, fallback)
            if asset then
                local ok, f = pcall(Font.new, asset, Enum.FontWeight.Regular, Enum.FontStyle.Normal)
                if ok and f then return f end
            end
            return Font.new(fallback, Enum.FontWeight.Regular, Enum.FontStyle.Normal)
        end

        fonts = {
            small = FontFace(Medium, Enum.Font.Gotham);
            font  = FontFace(SemiBold, Enum.Font.GothamMedium);
        }
    end
        ...
        function library:window(properties)
            local cfg = {
                ...
                name = properties.name or properties.Name or "vision";
                game_name = properties.gameInfo or properties.game_info or properties.GameInfo or "Vision";
            ...
            library.cache = library.cache or library[ "other" ]
...
do
    ...
    function library:Notify(text, dur)
        library.notifications:create_notification({ name = "Vision", info = tostring(text), lifetime = dur or 3 })
    end
