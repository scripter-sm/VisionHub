...
local Services = setmetatable({ game = game }, {
    __index = function(self, Key)
        local ok, Value = pcall(function() return cloneref(game:GetService(Key)) end)
        if ok and Value then rawset(self, Key, Value); return Value end
    end,
})
...
local delfile        = delfile        or delfolder or function() end
...
function library.configure(opts)
    opts = opts or {}
    if opts.directory then library.directory = opts.directory end
    if opts.folders then library.folders = opts.folders end
...
if type(GetImage) ~= 'function' then GetImage = function() return 'rbxassetid://0' end end
...
local is_mobile = _uis and _uis.TouchEnabled and not _uis.MouseEnabled or false
...
if not is_mobile then
    ...
    local fonts = {}; do
        ...
        function library:mouse_in_frame(uiobject)
            ...
            local mx, my = mouse.X, mouse.Y - gui_offset
            local y_cond = uiobject.AbsolutePosition.Y <= my and my <= uiobject.AbsolutePosition.Y + uiobject.AbsoluteSize.Y
            local x_cond = uiobject.AbsolutePosition.X <= mx and mx <= uiobject.AbsolutePosition.X + uiobject.AbsoluteSize.X
        ...
        function library:tab(properties)
            ...
            function cfg.open_tab()
                ...
                local single = #cfg.tabs <= 1
                library:tween(items[ "tab_holder" ], {Size = single and dim2(1, -196, 1, -41) or dim2(1, -196, 1, -81)}, Enum.EasingStyle.Quad, 0.4)
                items[ "tab_holder" ].Position = single and dim2(0, 196, 0, 16) or dim2(0, 196, 0, 56)

                items[ "tab_holder" ].Visible = true
                items[ "tab_holder" ].Parent = self.items[ "main" ]
                items[ "multi_section_button_holder" ].Visible = not single
        ...
        function library:list(properties)
            ...
            function cfg.refresh_options(options_to_refresh)
                local old_selected_value = flags[cfg.flag]
        ...
        function library:init_config(window)
            ...
            library.config_holder = section:list({options = {"Report", "This", "Error", "To", "Finobe"}, callback = function(option) end, flag = "config_name_list"}); library:update_config_list()
    ...
    local fonts = {}; do
        ...
        function library:init_config(window)
            ...
            library.config_holder = section:list({options = {"Report", "This", "Error", "To", "Finobe"}, callback = function(option)
                if text then
                    text.set(option)
                end
...
do
    ...
    function library:OnChange(fn)
        if type(fn) ~= "function" then return self end
        local prev = self.callback or function() end
        self.callback = function(...)
            prev(...)
            local ok, e = pcall(fn, ...)
            if not ok then warn("[Vision] callback error: " .. tostring(e)) end
        end
        if self.flag ~= nil and flags[self.flag] ~= nil then
            pcall(fn, flags[self.flag])
        end
        return self
    end
    ...
    function library:Set(v, a)
        if self.set then pcall(self.set, v, a) end
        return self
    end
    library.SetValue = library.Set

    function library:GetKey() return self.key end
    function library:SetKey(k)
        if self.set_key then pcall(self.set_key, k) end
        return self
    end
    ...
    local function makeElement(section, kind, o)
        ...
        local flag = opt(o, "Flag", "flag")
        local cb   = opt(o, "Callback", "callback")
        local name = opt(o, "Name", "name", "Text", "text")
        local dep  = opt(o, "Dependency", "dependency")
        local def  = opt(o, "Default", "default")
        local sep  = opt(o, "Seperator", "seperator", "Separator")
        local obj

        if kind == "toggle" then
            obj = section:toggle({ name = name, flag = flag, default = def or false, type = "toggle", callback = cb, seperator = sep })
        elseif kind == "slider" then
            local step = opt(o, "Step", "step", "Interval", "interval")
            local dec  = opt(o, "Decimals", "decimals")
            obj = section:slider({
                name = name, flag = flag, callback = cb,
                min = opt(o, "Min", "min", "Minimum", "minimum") or 0,
                max = opt(o, "Max", "max", "Maximum", "maximum") or 100,
                default = def or 0,
                interval = step or (dec and (10 ^ -dec)) or 1,
                suffix = opt(o, "Suffix", "suffix") or "",
                seperator = sep,
            })
        elseif kind == "dropdown" then
            obj = section:dropdown({
                name = name, flag = flag, callback = cb,
                items = opt(o, "Options", "options", "Items", "items") or {},
                multi = opt(o, "Multi", "multi") or false,
                default = def,
            })
        ...
        elseif kind == "button" then
            obj = section:button({ name = name or "button", callback = cb })
        end

        if obj then
            obj.__section = section
            applyDependency(obj, dep)
        end
        return obj or section
    end

    function library:Toggle(o)      return makeElement(self, "toggle", o) end
    function library:Slider(o)      return makeElement(self, "slider", o) end
    function library:Dropdown(o)    return makeElement(self, "dropdown", o) end
    function library:Textbox(o)     return makeElement(self, "textbox", o) end
    function library:Input(o)       return makeElement(self, "textbox", o) end
    function library:Keybind(o)     return makeElement(self, "keybind", o) end
    function library:Colorpicker(o) return makeElement(self, "colorpicker", o) end
    function library:Label(o)       return makeElement(self, "label", o) end
    ...
    function library:Tab(o)
        o = typeof(o) == "table" and o or { Name = tostring(o) }
        local name = o.Name or o.Title or o.name or "tab"
        local sub = self:tab({
            name = name,
            tabs = { name },
            icon = "http://www.roblox.com/asset/?id=6034767608",
        })
        local left, right = sub:column({}), sub:column({})
        local leftSecs, rightSecs = {}, {}
        local tabref = { __sub = sub, name = name }

        function tabref:Section(so)
            so = typeof(so) == "table" and so or { Name = tostring(so) }
            local side = string.lower(tostring(so.Side or so.side or "Left"))
            local col  = (side == "right") and right or left
            local list = (side == "right") and rightSecs or leftSecs
            local sec  = col:section({ name = so.Name or so.name or "section", side = side })
            list[#list + 1] = sec
            return sec
        end

        function tabref:ApplySettings() end
        function tabref:SetSettings() end

        if string.lower(name) == "settings" then
            local win = self
            local cfgDir = library.directory .. "/configs"
            pcall(makefolder, cfgDir)

            -- Configs (from idk.lua init_config)
            local secCfg = tabref:Section({ Name = "Configs", Side = "Left" })
            library.config_holder = secCfg:list({ options = {}, callback = function() end, flag = "config_name_list" })
            library:update_config_list()

            local secSet = tabref:Section({ Name = "Settings", Side = "Right" })
            secSet:textbox({ name = "Config name:", flag = "config_name_text" })
            secSet:button({ name = "Save", callback = function()
                pcall(function()
                    local n = flags["config_name_text"]
                    writefile(cfgDir .. "/" .. n .. ".cfg", library:get_config())
                    library:update_config_list()
                    library.notifications:create_notification({ name = "Configs", info = "Saved config to:\n" .. n })
                end)
            end })
            secSet:button({ name = "Overwrite", callback = function()
                pcall(function()
                    local n = flags["config_name_list"]
                    writefile(cfgDir .. "/" .. n .. ".cfg", library:get_config())
                    library:update_config_list()
                    library.notifications:create_notification({ name = "Configs", info = "Overwrote config :\n" .. n })
                end)
            end })
            secSet:button({ name = "Load", callback = function()
                pcall(function()
                    local n = flags["config_name_list"]
                    library:load_config(readfile(cfgDir .. "/" .. n .. ".cfg"))
                    library:update_config_list()
                    library.notifications:create_notification({ name = "Configs", info = "Loaded config:\n" .. n })
                end)
            end })
            secSet:button({ name = "Delete", callback = function()
                pcall(function()
                    local n = flags["config_name_list"]
                    delfile(cfgDir .. "/" .. n .. ".cfg")
                    library:update_config_list()
                    library.notifications:create_notification({ name = "Configs", info = "Deleted config:\n" .. n })
                end)
            end })
            ...
            local _request = (http_request and http_request) or (request and request) or (http and http.request)
            secSet:button({ name = "Join Lowest Server", callback = function()
                pcall(function()
                    local base = string.format("https://games.roblox.com/v1/games/%s/servers/Public?sortOrder=Asc&limit=100", tostring(game.PlaceId))
                    local function listServers(cursor)
                        return Services.HttpService:JSONDecode(game:HttpGet(base .. ((cursor and "&cursor=" .. cursor) or "")))
                    end
                    local server, nextc
                    repeat
                        local res = listServers(nextc)
                        server = res.data[1]
                        nextc = res.nextPageCursor
                    until server
                    if server.id == game.JobId then
                        library.library.notifications:create_notification({ name = "Vision", info = "You are already in the smallest server!", lifetime = 10 })
                        return
                    end
                    Services.TeleportService:TeleportToPlaceInstance(game.PlaceId, server.id)
                end)
            end })
            secSet:button({ name = "Server Hop", callback = function()
                pcall(function()
                    local servers = {}
                    local req = _request({ Url = string.format("https://games.roblox.com/v1/games/%d/servers/Public?sortOrder=Desc&limit=100&excludeFullGames=true", game.PlaceId) })
    ...
    function library:Notify(text, dur)
        library.library.notifications:create_notification({ name = "Vision", info = tostring(text), lifetime = dur or 3 })
    end
    library.Notification = library.Notify
    library.Notification2 = library.Notify

    function library:Log(...) return ... end
